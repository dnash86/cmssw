...
muIsoExtractorCalo_(0),muIsoExtractorTrack_(0),muIsoExtractorJet_(0)
{
   ...
   me0HitToken_ = consumes<ME0SegmentCollection>(me0Segments);
				...
				TrackDetectorAssociator::Direction direction)
{
   ...
   for ( const auto& chamber : info.chambers )
   {
     if (chamber.id.subdetId() == 3 && rpcHitHandle_.isValid()  ) continue; // Skip RPC chambers, they are taken care of below
       if  (chamber->id.subdetId() == MuonSubdetId::ME0 && me0Segments.isValid() ) continue;//Also skip ME0 chambers, they are taken care of below too
   ...
   if ( me0HitHandle_.isValid() )
       ...
       for ( const auto& chamber : info.chambers )
	 {
	   ...
	   for ( const auto& me0Segment : *me0HitHandle_ )
	     {
	       ...
	       if( (AbsDx <= 2 or AbsDx/sqrt(localError.xx() + me0Segment.localPositionError().xx()) <= 3) and 
		   (AbsDy <= 2 or AbsDy/sqrt(localError.yy() + me0Segment.localPositionError().yy()) <= 3)
