...
void CSCGeometryBuilder::build( boost::shared_ptr<CSCGeometry> theGeometry
  ...
  for ( size_t idt = 0; idt < detids.size(); ++idt) {
    ...
    //    LogTrace(myName) << "Chamber type = " << chamberType << std::endl;
    size_t cs = 0;
...
//     }
    ...
    //       LogTrace(myName) << " fu = " << fu << " going to maxFu = " << maxFu << std::endl;
    for ( ;fu < maxFu; ++fu ) {
      wg.consecutiveGroups.push_back(int(cscpars.pfupars[fu]));
    } 
          
    if ( wg.numberOfGroups != 0 ) {
      LogTrace(myName) << myName  << ": TotNumWireGroups     = " << wg.numberOfGroups  << std::endl;
      LogTrace(myName) << myName  << ": WireSpacing          = " << wg.wireSpacing  << std::endl;
      LogTrace(myName) << myName  << ": AlignmentPinToFirstWire = " << wg.alignmentPinToFirstWire  << std::endl;
      LogTrace(myName) << myName  << ": Narrow width of wire plane = " << wg.narrowWidthOfWirePlane  << std::endl;
      LogTrace(myName) << myName  << ": Wide width of wire plane = " << wg.wideWidthOfWirePlane  << std::endl;
      LogTrace(myName) << myName  << ": Length in y of wire plane = " << wg.lengthOfWirePlane  << std::endl;
      LogTrace(myName) << myName  << ": wg.consecutiveGroups.size() = " << wg.consecutiveGroups.size()  << std::endl;
      LogTrace(myName) << myName  << ": wg.wiresInEachGroup.size() = " << wg.wiresInEachGroup.size()  << std::endl;
      LogTrace(myName) << myName  << ": \tNumGroups\tWiresInGroup"  << std::endl;
      for (size_t i = 0; i < wg.consecutiveGroups.size(); i++) {
	LogTrace(myName) << myName  << " \t" << wg.consecutiveGroups[i] << "\t\t" << wg.wiresInEachGroup[i]  << std::endl;
      }
    } else {
      LogTrace(myName) << myName  << ": DDD is MISSING SpecPars for wire groups"  << std::endl;
    }
    LogTrace(myName) << myName << ": end of wire group info. "  << std::endl;
...
void CSCGeometryBuilder::buildChamber (  
  ...
  LogTrace(myName) << myName  << ": entering buildChamber"  << std::endl;
  ...
  LogTrace(myName) << myName  << ": gtran[0,1,2]=" << gtran[0] << " " << gtran[1] << " " << gtran[2]  << std::endl;
  ...
  if ( chamber ){
    ...
    LogTrace(myName) << myName <<": CSCChamberSpecs::build requested for ME" << jstat << jring  << std::endl;
...
//     if ( aSpecs == 0 ) aSpecs = theGeometry->buildSpecs( chamberType, fpar, fupar, wg );
	...
	LogTrace(myName) << "SHOULD BE THROW? Error, wg and/or fupar size are 0 BUT this Chamber Spec has not been built!" << std::endl;
    }
//  else if (fupar.size() != 0 && aSpecs != 0 ) {
//       LogTrace(myName) << "SHOULD BE THROW? Error, a Chamber Specs was found AND still the fupar and/or wg were/was non-zero! " << std::endl;
//     }
