...
# PSet of mixObjects that only keeps muon collections (and SimTracks with SimVertices) and tracker collections
mixObjects_dt_csc_rpc_tracker =  cms.PSet(
    mixCH = cms.PSet(
...
def customize_mix_muon_nocalo(process):
    process.mix.digitizers = digitizers = cms.PSet()
    digi_aliases = filter(lambda n: 'Digi' in n, process.aliases.keys())
    ...
    process.mix.mixObjects = mixObjects_dt_csc_rpc_tracker
    return process
...
# customize the full digitization sequence pdigi by adding GEMs and ME0s, but do NOT add calo
def customize_digi_addGEM_addME0_nocalo(process):
    process = customize_mix_muon_nocalo(process)
    process = load_GEM_digitizers(process)
    process = load_ME0_digitizers(process)
    process = customize_random_GEMDigi(process)
    process = customize_random_ME0Digi(process)
