...
unsigned int Muon::ME0layerMask( ArbitrationType type ) const
{
	 ...
	 chamberMatch != muMatches_.end(); chamberMatch++ )
   {
      ...
      //Not sure why the +=6, was copied from rpc...
      ...
      //May still be a bad implementation, ME0Segments have detId.layer() of 1 every time, with a roll of "-1", not sure how rpcs did it...
      if (region==1 && region==-1) {
	 me0Layer = chamberMatch->station()-1 + chamberMatch->station()*layer;
      }// else me0Layer += 6;   Probably not needed here
