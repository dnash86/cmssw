...
ME0MuonTrackCollProducer::ME0MuonTrackCollProducer(const edm::ParameterSet& parset) :
  ...
  parset_(parset),
  doTimeWindow(parset.getUntrackedParameter< bool >("doTimeWindow", false)),
  TimeWindowCut(parset.getUntrackedParameter< double >("TimeWindowCut", 12.5))
...
void ME0MuonTrackCollProducer::produce(edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  ...
  for(std::vector<reco::ME0Muon>::const_iterator thismuon = OurMuons->begin();
    ...
    if (!muon::isGoodMuon(me0Geom, *thismuon, muon::Tight)) continue;  //Tight muon
    if (doTimeWindow) {
      if (! ( ( thismuon->me0segment().time() >= (18.-TimeWindowCut)) &&
	      ( thismuon->me0segment().time() <= (18.+TimeWindowCut)) )
	  ) continue;
    }
